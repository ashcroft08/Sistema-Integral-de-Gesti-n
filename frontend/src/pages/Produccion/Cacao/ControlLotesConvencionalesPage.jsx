...
const ControlLotesConvencionalesPage = ({ onBack }) => {
    ...
    const handleConfirmSeco = async (lote) => {
        try {
            await controlLoteCvService.update(lote.id_control_lote_cv, { estado: 'SECO' });
            toast.success('Estado del lote cambiado a SECO correctamente', { toastId: 'seco-mark-cv' });
            fetchLotes();
        } catch (err) {
    ...
    const handleUpdateRuta = async (id, id_ruta_compra) => {
        try {
            await controlLoteCvService.update(id, { id_ruta_compra });
            toast.success('Ruta asignada correctamente', { toastId: 'ruta-update-cv' });
            fetchLotes();
        } catch (err) {
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                            ...
                            <div className="rounded-xl border border-primary/10 dark:border-primary/20 bg-white/60 dark:bg-background-dark/40 backdrop-blur-sm p-4 shadow-sm hover:scale-[1.01] transition-all flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <span className="text-[10px] font-bold text-text-secondary dark:text-background-light/50 uppercase tracking-wider">Total Comercializado (Lbs)</span>
                                </div>
                            </div>
                            ...
                            <div className="rounded-xl border border-primary/10 dark:border-primary/20 bg-white/60 dark:bg-background-dark/40 backdrop-blur-sm p-4 shadow-sm hover:scale-[1.01] transition-all flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <span className="text-[10px] font-bold text-text-secondary dark:text-background-light/50 uppercase tracking-wider">Total Comercializado (QQ)</span>
                                </div>
                            </div>
                        </div>
                    )}
