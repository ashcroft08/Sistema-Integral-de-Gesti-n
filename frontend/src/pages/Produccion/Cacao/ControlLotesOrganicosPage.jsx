...
const ControlLotesOrganicosPage = ({ onBack }) => {
    ...
    const fetchLotes = useCallback(async () => {
        ...
        try {
            ...
            const response = await controlLoteOrgService.getAll(selectedPeriod);
            setLotes(response.data || []);
        } catch (err) {
            toast.error('Error al cargar la información de los lotes');
        } finally {
    ...
    const handleUpdateOdp = async (id, odp) => {
        try {
            await controlLoteOrgService.update(id, { odp });
            toast.success('ODP actualizada correctamente', { toastId: 'odp-update-org' });
            fetchLotes();
        } catch (err) {
            toast.error('Error al actualizar ODP');
        }
    };
    ...
    const handleSaveComercializacion = async (data) => {
        try {
            await controlLoteOrgService.saveComercializacion(data);
            toast.success('Clasificación comercial guardada con éxito', { toastId: 'comercializacion-save-org' });
            if (data.porcentaje_perdida > 3) {
                toast.warning(`Advertencia: El porcentaje de pérdida (${data.porcentaje_perdida}) supera el límite de 3. Por favor, revise el lote físico.`, { 
                    toastId: `comercializacion-warn-org-${data.id_control_lote_org}`,
                    autoClose: 10000 
                });
            }
            fetchLotes();
    ...
    const renderContent = () => (
        <div className="space-y-6">
            {/* Breadcrumb */}
            <Breadcrumbs onBack={onBack ? onBack : () => navigate('/produccion')} currentPath="Control Lotes Orgánicos" />
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-2">
                <div>
                    <h1 className="text-2xl font-heading font-bold text-text-primary dark:text-background-light tracking-tight flex items-center gap-3">
                        ...
                        Control de Lotes Orgánicos
                    </h1>
                    <p className="text-sm text-text-secondary dark:text-background-light/50 mt-1">
                        Monitorea y clasifica lotes orgánicos procesados, gestiona ODPs y registra su distribución de comercialización.
                    </p>
                </div>
            </div>
            ...
            {!selectedPeriod ? (
                <div className="rounded-2xl border border-primary/10 dark:border-primary/20 bg-white/40 dark:bg-background-dark/30 py-16 px-6 text-center shadow-sm">
                    ...
                    <p className="text-sm text-text-secondary/70 dark:text-background-light/40 max-w-md mx-auto">
                        Selecciona un período de compra arriba para visualizar y administrar la trazabilidad de los lotes orgánicos.
                    </p>
                </div>
            ) : (
                <>
...
export default ControlLotesOrganicosPage;
